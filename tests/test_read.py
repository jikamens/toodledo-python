...
def test_get_tasks_with_known_folders(toodledo):
    ...
    taskWithPrivateFolder = [t for t in tasks
                             if t.title == "Test task with private folder"][0]
    taskWithPublicFolder = [t for t in tasks
                            if t.title == "Test task with public folder"][0]
...
def test_get_tasks_with_known_contexts(toodledo):
    ...
    taskWithPrivateContext = [
        t for t in tasks if t.title == "Test task with private context"][0]
    taskWithPublicContext = [
        t for t in tasks if t.title == "Test task with public context"][0]
    privateContextId = [
